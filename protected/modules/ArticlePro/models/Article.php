...
class Article extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('uid, title, author, isallow, uploadtime, content, typeid', 'required'),
			array('isallow', 'numerical', 'integerOnly'=>true),
			array('uid, typeid', 'length', 'max'=>10),
			array('title, fromlink', 'length', 'max'=>128),
			array('from', 'length', 'max'=>64),
			array('author', 'length', 'max'=>32),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('aid, uid, title, from, fromlink, author, isallow, uploadtime, content, typeid', 'safe', 'on'=>'search'),
		);
	}
